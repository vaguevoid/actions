...
OUTPUT_FILE=$(mktemp)
STATUS_CODE=$(curl -s -X POST -o $OUTPUT_FILE -w "%{http_code}" --connect-timeout 180 --max-time 300 -H "X-Deploy-Label: ${label}" -H "X-Deploy-Password: ${password}" -H "X-Deploy-Pinned: ${pinned}" -H "Authorization: Bearer ${token}" -T "${bundle}" ${endpoint} 2>/dev/null)
